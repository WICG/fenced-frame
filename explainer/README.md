...
*   **Navigation url:** Since fenced frames are allowed to open popups (or navigate the top-level page in some modes), the navigation url can carry bits of information out of the fenced frame tree. Based on the isolation of fenced frames from the embedding context, the goal is that this should not be sufficient for joining cross-site information within the fenced frame and the embedder's information. Having said that, this is vulnerable to the network side channel as mentioned above or to correlation of other information such as fenced frame creation time. Information joining is also reduced by gating these on user activation. 
